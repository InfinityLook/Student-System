...
export const ModuleLauncher: React.FC = () => {
  ...
  type Tile = { id: string; name: string; icon: typeof Lock; color: string; kind: TileKind };
    ...
    <div className="glass-panel p-5 rounded-2xl border border-studypilot-border space-y-4">
      ...
      <div className="grid grid-cols-3 gap-3 max-w-xs mx-auto">
        {gridItems.map((item, idx) => {
          ...
          const isLocked = tile.kind === 'planned' || tile.kind === 'empty';
          ...
          return (
            <motion.button
              ...
              onClick={() => {
                if (tile.kind === 'active' || tile.kind === 'available') toggleModule(tile.id);
              }}
              disabled={isLocked}
              ...
              {isLocked && !tile.name ? (
                ...
                <>
                  {Icon && (
                    <Icon
                      className="w-5 h-5"
                      style={{ color: isLocked ? '#5b5f75' : tile.color }}
                    />
                  )}
                  <span className={`text-[9px] font-medium text-center leading-tight px-1 ${isLocked ? 'text-gray-600' : 'text-gray-300'}`}>
                    {isLocked && <Lock className="w-2.5 h-2.5 inline mr-0.5 -mt-0.5" />}
                    {tile.name}
                  </span>
                </>
              )}
